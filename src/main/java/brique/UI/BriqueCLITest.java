...
/**
 * Unit tests for {@link BriqueCLI}.  These tests simulate user interaction
 * by supplying pre‑defined input strings through a {@link TestIO} and
 * then verifying both the resulting game state (through the underlying
 * {@link GameEngine}) and the textual output captured from the CLI.  The
 * scenarios cover normal move sequences, the pie rule, invalid input and
 * game termination.
 */
class BriqueCLITest {
